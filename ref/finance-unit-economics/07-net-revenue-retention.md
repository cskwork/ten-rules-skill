# Rule 07 — Grow Net Revenue Retention Past 100%

> 신규 고객 없이도 매출이 늘어난다면, 그것이 진짜 엔진이다.
> If revenue grows without any new customers, that is the real engine.

## Why

Net revenue retention (NRR) measures how revenue from an existing cohort changes over a period after expansion, contraction, and churn. Above 100 percent means the base grows on its own, compounding every future cohort and easing the burden on acquisition. Bessemer benchmarks treat NRR of 100 percent as good, 120 percent or more as best, and it is among the strongest drivers of valuation. High NRR also signals durable product value, which validates the LTV assumptions behind every other rule.

## How to apply

- Measure NRR on a fixed cohort: starting recurring revenue plus expansion minus contraction minus churn.
- Separate gross retention (no expansion credit) from net retention to see churn clearly.
- Build expansion paths (upsell, usage growth, seats) so accounts can grow over time.
- Attack involuntary churn (failed payments) and early churn, which quietly erode the base.
- Use NRR to ground the lifetime assumptions in LTV (Rule 02) and cohorts (Rule 08).

## Checklist

- [ ] NRR is computed on a fixed starting cohort
- [ ] Gross and net retention are both reported
- [ ] NRR is benchmarked against the 100 to 120 percent range
- [ ] Expansion and contraction are broken out, not netted silently
- [ ] Involuntary churn is tracked and actively recovered

## References

- Bessemer Venture Partners Atlas — https://www.bvp.com/atlas
- forEntrepreneurs (David Skok) — https://www.forentrepreneurs.com/saas-metrics-2-definitions-2/
